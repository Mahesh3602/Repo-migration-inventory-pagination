...
#
#  Design:
#  - Build/YAML pipelines : project-wide fetch using queryOrder=3 (nameAscending)
#    Pagination via x-ms-continuationtoken response header — confirmed via Postman.
#    Token passed back raw (no URL encoding) — confirmed working with Spanish chars.
#    One definition/{id} call per pipeline to resolve repository name and
#    pipeline type (YAML vs Classic Build) — both absent from list response.
#    Duplicate pipeline IDs skipped via seenIds hashtable (token boundary safety).
#  - Classic Release pipelines: project-wide fetch via vsrm host.
#    Pagination via x-ms-continuationtoken header — works without queryOrder.
#
#  PipelineType values : "YAML" | "Classic Build" | "Classic Release"
#
#  Output file naming (automatic based on run mode):
#  - Build + YAML run  : *_build.csv / *_build_errors.log
#  - Release only run  : *_release.csv / *_release_errors.log
#  - Full run (default): *_all.csv / *_all_errors.log
#
#  Key design guarantees:
#  - Pre-flight validation: checks Org, PAT and API reachability before any work
#  - Both -SkipBuildPipelines and -SkipReleasePipelines together rejected at startup
#  - All pagination uses while($true) + explicit break — never do..while($token)
#  - Build definitions uses queryOrder=3 — REQUIRED for continuationToken to work
#  - Token read from x-ms-continuationtoken header; body continuationToken as fallback
#  - Token passed back raw — no URL encoding — safe for Spanish/accented names
#  - Duplicate pipeline IDs deduplicated via seenIds hashtable
#  - Empty-value pages always break regardless of token presence (infinite loop guard)
#  - Safety page cap of 1000 pages (covers 100k items per scope at 100/page)
#  - Console output throttled — only project-level progress shown at scale
#  - Retry/throttle handles PS5 and PS Core header differences (string vs string[])
#  - CSV encoding: UTF8NoBOM on PS Core, UTF8 on PS5
# ============================================================
...
param (
    ...
    [switch]$SkipReleasePipelines = $true,
    [switch]$SkipBuildPipelines   = $false
)
...
if ($SkipBuildPipelines -and $SkipReleasePipelines) {
    Write-Error "Cannot use both -SkipBuildPipelines and -SkipReleasePipelines together — nothing to fetch."
    exit 1
}

# =============================================================================
#  OUTPUT FILE NAMING — suffix driven by run mode
# =============================================================================
$suffix = if ($SkipReleasePipelines)  { "_build"   }
          elseif ($SkipBuildPipelines) { "_release" }
          else                         { "_all"     }

$PipelineCsv     = "ado_pipeline_inventory${suffix}.csv"
$RepoCsv         = "ado_pipeline_repo_counts${suffix}.csv"
$ProjectCountCsv = "ado_pipeline_project_counts${suffix}.csv"
$ErrorLog        = "ado_pipeline_inventory${suffix}_errors.log"
...
Write-Host "  Mode       : $($suffix.TrimStart('_').ToUpper())"
Write-Host "  Build      : $(if ($SkipBuildPipelines)   { 'skipped' } else { 'included' })"
Write-Host "  Releases   : $(if ($SkipReleasePipelines) { 'skipped' } else { 'included' })"
Write-Host "  Output     : $PipelineCsv`n"

# =============================================================================
#  HELPER: safely read a header value on PS5 (string) and PS Core (string[])
# =============================================================================
...
# =============================================================================
#  CORE API WRAPPER
#  - 429/503 throttling with Retry-After and exponential backoff
#  - Token: x-ms-continuationtoken header first, body continuationToken fallback
#  - Token stored raw — no encoding applied
#  - PS5/PS Core safe
# =============================================================================
...
function Write-ErrorLog {
    ...
    # Intentionally not mirroring to console — errors logged to file for post-run review
}

# =============================================================================
#  BUILD PIPELINE FETCH
#
#  PAGINATION FIX: queryOrder=3 (nameAscending) is REQUIRED on every call.
#  Without it the API returns a timestamp-based token which ADO rejects as
#  "ambiguous" on the next page — confirmed root cause of the original
#  page 2 skip bug. With queryOrder=3 the token is a pipeline name string
#  which works reliably including for Spanish/accented characters.
#
#  Token is in x-ms-continuationtoken response header (not body).
#  Token passed back raw — no URL encoding — confirmed safe via Postman.
#  seenIds hashtable deduplicates pipelines at token page boundaries.
#
#  Individual definition/{id} fetch per pipeline resolves:
#  - repository.name  : absent from list response (confirmed Postman)
#  - process.type     : 1 = Classic Build, 2 = YAML (absent from list response)
#  On detail fetch failure: pipeline still written with blank repo and
#  "Build Pipeline" fallback type — never silently skipped.
#
#  Phase 1 — API only : collect all rows in memory
#  Phase 2 — Write only: flush all rows to CSV
# =============================================================================
function Get-BuildPipelinesForProject {
    param (
        [string]$ProjectName,
        [System.Collections.Generic.List[object]]$AllRepos
    )
    ...
    while ($true) {
        ...
        # queryOrder=3 is REQUIRED — do not remove. See function header comment.
        $url = "https://dev.azure.com/$script:Org/$([Uri]::EscapeDataString($ProjectName))/_apis/build/definitions" +
               "?`$top=100&queryOrder=3&api-version=7.1"
        ...
        foreach ($def in $defs) {
            # Deduplicate — token page boundary can return same pipeline twice
            if ($seenIds.ContainsKey($def.id)) { continue }
            $seenIds[$def.id] = $true
            $totalCount++

            $repoName     = ""
            $pipelineType = "Build Pipeline"   # fallback if detail fetch fails

            $detailUrl = "https://dev.azure.com/$script:Org/$([Uri]::EscapeDataString($ProjectName))/_apis/build/definitions/$($def.id)?api-version=7.1"
            Wait-PoliteDelay
            $detail = Invoke-ADOApi -Url $detailUrl

            if ($detail -and $detail.Body) {
                if ($detail.Body.repository -and $detail.Body.repository.name) {
                    $repoName = $detail.Body.repository.name
                }
                if ($detail.Body.process -and $detail.Body.process.PSObject.Properties["type"]) {
                    $pipelineType = switch ($detail.Body.process.type) {
                        1       { "Classic Build" }
                        2       { "YAML"          }
                ...
                Write-ErrorLog "SKIPPED detail fetch for pipeline '$($def.name)' (id=$($def.id)) in '$ProjectName' — API returned null. Written with blank repo and fallback type."
            }
            ...
            $pipelineRows.Add([PSCustomObject]@{
                ...
                PipelineUrl  = if ($def._links -and $def._links.web) { $def._links.web.href } else { "" }
            })
        }

        # No token = last page. Token present = more pages exist.
...
# =============================================================================
#  RELEASE PIPELINE FETCH
#
#  Classic Release pipelines via vsrm host.
#  Pagination via x-ms-continuationtoken header — works without queryOrder.
#  Repository column blank — release pipelines not tied to a single repo.
#
#  Phase 1 — API only : collect all rows in memory
#  Phase 2 — Write only: flush all rows to CSV
# =============================================================================
function Get-ReleasePipelinesForProject {
    ...
    while ($true) {
        ...
        if (-not $result -or -not $result.Body) {
            Write-ErrorLog "SKIPPED release pipelines (page $page) for '$ProjectName' — API returned null."
            break
        }
...
# =============================================================================
#  STEP 2 — Per project: repos → build pipelines → release pipelines
# =============================================================================
$totalPipelines = 0
$totalRepos     = 0
$failedProjects = 0
...
foreach ($proj in $allProjects) {
    ...
    # ── 2b. Build pipelines ───────────────────────────────────────────────────
    if (-not $SkipBuildPipelines) {
        if ($allRepos.Count -eq 0) {
            Write-Host "  No repos found — skipping build pipeline fetch" -ForegroundColor Gray
            $buildCount = 0
        } else {
            Write-Host "  Fetching build pipelines..." -ForegroundColor Gray
    ...
    if (-not $SkipReleasePipelines) {
        Write-Host "  Fetching release pipelines..." -ForegroundColor Gray
        $relCount       = Get-ReleasePipelinesForProject -ProjectName $projName
        $projPipelines += $relCount
        $totalPipelines += $relCount
        Write-Host "  Release pipelines: $relCount" -ForegroundColor Gray
    }

    # ── Project summary — always written ──────────────────────────────────────
